# ocr_manager.py
# ★★★ 修正: Windowsでのパス引用符エラーを回避するため、環境変数方式に変更 ★★★
...
# アプリのロケールコードとTesseractのデータファイル名の対応表
LOCALE_TO_TESS_CODE = {
    "en_US": "eng",
    "ja_JP": "jpn",
    "de_DE": "deu",
...
class OCRWorker(QThread):
    ...
    def run(self):
        try:
            # ★★★ 修正: 環境変数 TESSDATA_PREFIX をセットする方式に戻す ★★★
            # これによりパスの引用符問題を回避
            if self.data_dir:
            ...
            # --tessdata-dir オプションは削除 (環境変数と競合するため)
            
            # 数値モード時のホワイトリスト設定 (高速化)
            ...
            self.finished.emit(raw_text, numeric_value)
...
class OCRManager(QObject):
    ...
    def __init__(self):
        ...
        # ホームディレクトリ直下の click_pic/tessdata を参照
    ...
    def run(self):
        ...
        for lang in self.target_langs:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    with open(path, 'wb') as f:
                        if total_len is None:
                            ...
                            for chunk in res.iter_content(chunk_size=CHUNK_SIZE):
                                dl += len(chunk)
                                f.write(chunk)
                                if dl % (CHUNK_SIZE * 20) == 0:
                                    self.progress.emit(fname, int(100 * dl / total_len))
                    ...
                    if attempt < MAX_RETRIES - 1:
                        print(f"[OCR DL Retry] {fname}: {e} (Retrying in {RETRY_DELAY}s...)")
            ...
            if download_success:
                ...
                print(f"[OCR DL Fail] {fname}: {last_error}")
                if os.path.exists(path):
                    try: os.remove(path)
                    except: pass
